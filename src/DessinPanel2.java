...
import states.Panel;
import states.AucuneSelection;
import states.EtatSelection;
import states.ToutSelection;
import states.UneSelection;
 ...
 * 
 */
public class DessinPanel2 extends JPanel implements Panel{
	...
	public void paintComponent(Graphics g) {
		...
		for (FormGeo f : formesGeo) {
			f.dessine(g2);
			etatCourant.dessiner(this, selectedFormesGeo, g2);
			/*if (courant != null) {
	...
	private void setEtatCourant(){
		if(selectedFormesGeo.isEmpty()){
			etatCourant = new AucuneSelection();
		}
		if(selectedFormesGeo.size() == 1){
			etatCourant = new UneSelection();
		}
		if(selectedFormesGeo.size() > 1){
			etatCourant = new ToutSelection();
		}
	}
		...
		public void mouseDragged(MouseEvent event) {
			...
			if (courant == null) {
				...
				try {
					...
					for (FormGeo f : selectedFormesGeo) {
						f.moveBy(dx, dy);
						System.out.println("TS" + Toutselec);
						if (Toutselec != 1) {
							// Pour arreter la selection de plusieurs fichiers
